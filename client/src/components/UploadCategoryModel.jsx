...
const UploadCategoryModel = ({close,fetchData}) => {
  ...
  return (
   <section className='fixed top-0 bottom-0 left-0 right-0 p-4 flex items-center justify-center'>
     <div className='bg-gray-200 max-w-5xl w-full p-4 rounded'>
       <div className='flex items-center justify-between'>
        ...
        <form className='my-3 grid gap-2' onSubmit={handleSubmit}>
          ...
          <button className={`${data.name && data.image ? "bg-amber-400 py-2 font-semibold":"bg-gray-400 py-2"}`} >Add Category</button>
        </form>
     </div>
   </section>
  )
